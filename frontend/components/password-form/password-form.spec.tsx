...
describe('PasswordForm', () => {
  it('renders password input and submit button', () => {
    render(<PasswordForm onSubmit={jest.fn()} text="Export" loadingText="Exporting" />)
    ...
    expect(screen.getByTestId(locators.passphraseSubmit)).toHaveTextContent('Export')
  })
  ...
  it('shows incorrect password text if password is entered incorrectly', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('input-error-text')).toHaveTextContent('Incorrect passphrase'))
  })
  ...
  it('shows message unknown error occurs', async () => {
    ...
    fireEvent.change(passwordInput, { target: { value: 'password123' } })
    fireEvent.click(screen.getByTestId(locators.passphraseSubmit))
    await waitFor(() => expect(screen.getByTestId('input-error-text')).toHaveTextContent('Unknown error occurred: foo'))
  })
})
