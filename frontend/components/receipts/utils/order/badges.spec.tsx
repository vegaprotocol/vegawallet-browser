...
describe('OrderBadges component', () => {
  test('renders Good til date badge when timeInForce is GTT', () => {
    // 1119-ORSH-004 If time in force is GTT then I can see the expiry of the order
  ...
  test('renders TIF badge when timeInForce is not GTT', () => {
    // 1119-ORSH-003 I can see a badge of the order time in force
    render(
      <OrderBadges
        postOnly={false}
        reduceOnly={false}
  ...
  test('renders post only badge when postOnly is true', () => {
    // 1119-ORSH-001 I can see a badge if the order is post only
    render(
      <OrderBadges
        postOnly={true}
        reduceOnly={false}
  ...
  test('renders reduce only badge when reduceOnly is true', () => {
    // 1119-ORSH-002 I can see a badge if the order is reduce only
    render(
      <OrderBadges
        postOnly={false}
        reduceOnly={true}
