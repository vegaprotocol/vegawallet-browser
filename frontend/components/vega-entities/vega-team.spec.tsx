...
describe('VegaTeam', () => {
  ...
  it('should show the team id while loading and team name when loading is completed', async () => {
    ...
    mockRequest.mockReturnValue(new Promise((resolve) => resolve({ teams: { edges: [{ node: { name: 'test' } }] } })))
    ...
    await waitFor(() => expect(screen.getByTestId(locators.teamLink)).toHaveTextContent('test'))
  })
  it('should show the id if there is an error', async () => {
    silenceErrors()
    const mockRequest = jest.fn()
    mockRequest.mockReturnValue(new Promise((resolve, reject) => reject(new Error('test error'))))
    ;(useJsonRpcClient as unknown as jest.Mock).mockReturnValue({ request: mockRequest })
    renderComponent()
    expect(screen.getByTestId(locators.teamLink)).toHaveTextContent('1')
    await waitFor(() => expect(screen.getByTestId(locators.teamLink)).toHaveTextContent('1'))
  })
})
