...
import { InteractionStore, useInteractionStore } from '@/stores/interaction-store'
...
describe('NetworkProvider', () => {
  it('throws an error if a component is rendered inside of it', () => {
    silenceErrors()
    expect(() => render(<TestComponent />)).toThrow('useNetwork must be used within NetworkProvider')
  })
  ...
  it('returns the network that is selected in settings by default', () => {
    ...
    expect(screen.getByTestId('test')).toHaveTextContent(JSON.stringify(testingNetwork))
  })
  ...
  it('if we are connecting to a dApp then the chainId of the connection is used to find the network', () => {
    ...
    expect(screen.getByTestId('test')).toHaveTextContent(JSON.stringify(fairground))
  })
