...
import { ExportRecoveryPhraseSection, locators } from './export-recovery-phrase-section'
import { ViewRecoveryPhraseProperties } from './view-recovery-phrase'

jest.mock('./export-recovery-phrase-form', () => ({
  ExportRecoveryPhraseForm: (properties: ExportRecoveryPhraseFromProperties) => (
...
describe('ExportRecoveryPhraseSection', () => {
  it('renders export button', () => {
    ...
    expect(screen.getByTestId(locators.exportRecoveryPhraseTrigger)).toBeInTheDocument()
  ...
  it('open dialog when export button is pressed with title', async () => {
    ...
    expect(screen.getByTestId(locators.exportRecoveryPhraseTitle)).toHaveTextContent('Export Recovery Phrase')
  })
  it('renders form when private key has yet to be loaded', async () => {
    ...
    fireEvent.click(screen.getByTestId(locators.exportRecoveryPhraseTrigger))
    await screen.findByTestId(locators.exportRecoveryPhraseTitle)
    expect(screen.getByTestId('export-recovery-phrase-form')).toBeInTheDocument()
  })
  ...
  it('resets asset dialog on close of form', async () => {
    ...
    expect(screen.queryByTestId(locators.exportRecoveryPhraseTitle)).not.toBeInTheDocument()
    fireEvent.click(screen.getByTestId(locators.exportRecoveryPhraseTrigger))
    await screen.findByTestId(locators.exportRecoveryPhraseTitle)
    expect(screen.getByTestId('export-recovery-phrase-form')).toBeInTheDocument()
  })
