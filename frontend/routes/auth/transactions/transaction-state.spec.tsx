...
describe('TransactionState', () => {
  it('renders info color when transaction state is confirmed', () => {
    ...
    expect(screen.getByTestId(locators.transactionState)).toHaveClass('text-intent-info-background')
  })
  it('renders error color when transaction state is error', () => {
    renderComponent(TransactionState.Error)
    expect(screen.getByTestId(locators.transactionState)).toHaveClass('text-intent-danger-background')
  })
