...
export default function init(sleep = 45 * 60 * 1000, ping = 1000) {
  ...
  // prevent the extension from going to sleep. It is implemented
  // as a loop of setTimeouts instead of a setInterval to prevent
  // queueing up ping messages.
  ...
  return function keepAlive(port) {
    ...
    keepAliveInterval = setTimeout(function loop() {
      try {
        port.postMessage({ jsonrpc: '2.0', method: 'ping', params: null })
        keepAliveInterval = setTimeout(loop, ping)
      } catch (error) {
