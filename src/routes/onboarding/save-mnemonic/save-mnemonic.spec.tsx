...
describe('Save mnemonic', () => {
  ...
  it('mnemonic and checkbox are shown when clicked', async () => {
    // 1101-BWAL-014 I can see an explanation of what the recovery phrase is for and that it cannot be recovered itself
    renderComponent()
    await screen.findByTestId(locators.mnemonicContainerHidden)
  ...
  it('submit button is disabled if checkbox or recovery phrase is not revealed and checked, enabled if revealed and checked', async () => {
    // 1101-BWAL-017 I can verify that I understand that Vega doesn't store and therefore can't recover this recovery phrase if I lose it
    renderComponent()
    await screen.findByTestId(locators.mnemonicContainerHidden)
