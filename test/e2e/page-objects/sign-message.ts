...
export class SignMessage {
  ...
  async checkMessageSignedSuccessfully() {
    expect(await isElementDisplayed(this.driver, this.signedMessageText), 'could not locate a signed message', {
      showPrefix: false
    }).toBe(true)

    const signedText = await getElementText(this.driver, this.signedMessageText)
    expect(signedText, 'signed message text was empty', { showPrefix: false }).toBeTruthy()
  }

  async checkMessageSignedAndClose() {
    await this.checkMessageSignedSuccessfully()
