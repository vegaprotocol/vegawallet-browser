...
describe('transactions', () => {
  ...
  it('can confirm a transaction', async () => {
    // 1101-BWAL-046 When I view a transaction request I can choose to approve it
  ...
  it('the result of the transaction request is fed back to the UI', async () => {
    // 1101-BWAL-047 When I approve a transaction the transaction gets signed and the approved status gets fed back to the dapp that requested it
    const keys = await vegaAPI.listKeys()
    await vegaAPI.sendTransaction(keys[0].publicKey, { transfer: transferReq })
    await transaction.checkOnTransactionPage()
    await transaction.confirmTransaction()
  ...
  it('queues transactions when there is more than one', async () => {
    // 1101-BWAL-048 When I approve a transaction after I have approved it we revert to the next transaction if there's a queue OR we revert to the key view (the front / homepage)
  ...
  it('can reject a transaction', async () => {
    // 1101-BWAL-049 When I view a transaction request I can choose to reject it
