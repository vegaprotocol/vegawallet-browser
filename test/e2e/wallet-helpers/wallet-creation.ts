...
export class CreateWallet {
  private readonly landingPageURL: string = "chrome-extension://jfaancmgehieoohdnmcdfdlkblfcehph/index.html"; // This is assuming the create new wallet is the landing page
  private readonly createNewWalletButton: By = By.id(locators.createNewWalletButton);
  private readonly passwordInput: By = By.id(locators.passwordInput);
  private readonly confirmPasswordInput: By = By.id(locators.confirmPasswordInput);
  ...
  private readonly revealRecoveryPhraseButton: By = By.id(locators.revealRecoveryPhraseButton);
  private readonly copyRecoveryPhraseToClipboardButton: By = By.id(locators.copyRecoveryPhraseToClipboardButton);
  private readonly acknowledgeRecoveryPhraseWarningCheckbox: By = By.id(locators.recoveryPhraseWarningCheckbox);
  private readonly secureWalletContinueButton: By = By.id(locators.secureWalletContinueButton);
  ...
  async navigateToLandingPage() {
    ...
    expect(await this.isGettingStartedPage(), this.checkOnCorrectViewErrorMessage("Getting Started")).toBe(true);
  }
  ...
  async secureNewWallet(acknowledgeWarning: boolean = true) {
    expect(await this.isSecureWalletPage(), this.checkOnCorrectViewErrorMessage("Secure Wallet")).toBe(true);
