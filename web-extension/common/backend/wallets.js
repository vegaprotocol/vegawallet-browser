...
import { generate as generateMnemonic } from '@vegaprotocol/crypto/lib/bip-0039/mnemonic'
...
export class WalletCollection {
  ...
  async create ({ name }) {
    ...
    try {
      ...
      if (await this.store.has(name)) throw new Error(`Wallet with name "${name}" already exists.`)
      ...
      release()
      await this.generateKey({ wallet: name, metadata: [{ key: 'name', value: 'Key 1' }] })
    }
  }
